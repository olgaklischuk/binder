...
library(vars)
# for(index in 1:6){
  i=c(11:12,14:15,"15.extension",16)[3];directory=pst(OneDrive,"I");if(i==0){estmod="direct"; varchannel=get(paste("varchannelNBU",sep=""));Amat=as.matrix(get(paste("amat",i,sep=".")));Bmat=as.matrix(get(paste("bmat",i,sep=".")));directory=paste(OneDrive,"I",sep="");direct=TRUE;write.xlsx=TRUE;names.plot<-(get(paste("NAMES",i,"U",sep=".")));im<-1}else{
...
if(is.null(names.plot)){if(is.null(names.plot.2)){
  names.plot<-get(paste("NAMES",n_2,"U",sep="."))
  names.plot.2<-get(paste("NAMES",n_2,"U.2",sep="."))}else{names.plot<-get(paste("NAMES",n_2,"U",sep="."));names.plot.2<-names.plot.2}
}else{if(is.null(names.plot.2)){names.plot<-names.plot;names.plot.2<-get(paste("NAMES",n_2,"U.2",sep="."))}else{names.plot<-names.plot;names.plot.2<-names.plot.2}}
...
START<-Sys.time()
ASvar<-try(SVAR(varchannel, estmethod = estmod,#"direct" "scoring" ),
            Amat = Amat, Bmat = NULL,
            start = rep(.01,id$params.A), max.iter = 1000, conv.crit = 1e-07, maxls = 2,
            lrtest = TRUE))
BSvar<-try(SVAR(varchannel, estmethod = estmod,#"direct" "scoring" ),
            Amat = NULL, Bmat = Bmat ,
...
Asvar.coef<-foreach(i=1:length(names(varchannel$varresult)),.combine=rbind)%do%(ASvar$var$varresult[[i]]$coefficients)
Bsvar.coef<-foreach(i=1:length(names(varchannel$varresult)),.combine=rbind)%do%(BSvar$var$varresult[[i]]$coefficients)
ABsvar.coef<-foreach(i=1:length(names(varchannel$varresult)),.combine=rbind)%do%(ABSvar$var$varresult[[i]]$coefficients)
...
if(n==1){j<-n} else j<-paste(n,".1",sep=""); if(!dir.exists(paste(dir,"/svar.code/result5/svarchannel",j,"/plot.irf",sep=""))){dir.create(paste(dir,"/svar.code/result5/svarchannel",j,sep=""));dir.create(paste(dir,"/svar.code/result5/svarchannel",j,"/plot.irf",sep=""))};if(!getwd()==paste(dir,"/svar.code/result5/svarchannel",j,"/plot.irf",sep=""))setwd(paste(dir,"/svar.code/result5/svarchannel",j,"/plot.irf",sep=""))
...
if(!dir.exists(paste(dir,"/svar.code/result5/svarchannel",j,"/plot.fevd",sep=""))){dir.create(paste(dir,"/svar.code/result5/svarchannel",j,sep=""));dir.create(paste(dir,"/svar.code/result5/svarchannel",j,"/plot.fevd",sep=""))};if(!getwd()==paste(dir,"/svar.code/result5/svarchannel",j,"/plot.fevd",sep=""))setwd(paste(dir,"/svar.code/result5/svarchannel",j,"/plot.fevd",sep=""))
