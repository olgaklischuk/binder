...
library(vars)
i=0;directory=pst(OneDrive,"I");if(i==0){estmod="direct"; varchannel=get(paste("varchannelNBU",sep="")); Amat=as.matrix(get(paste("amat",i,sep="."))); Bmat=as.matrix(get(paste("bmat",i,sep="."))); directory=paste(OneDrive,"I",sep=""); direct=TRUE; write.xlsx=TRUE; names.plot<-(get(paste("NAMES",i,"U",sep="."))); im<-1}else{
...
par(cex.lab=.97,lheight=1.1)
title<-paste("Реакція макрозмінних","на шок",names.plot[im], "за SVAR моделлю",n+1)
png(paste("Fig",j,"7.A model.png",sep="."),type="cairo-png")#names.=names.plot
plot1<-plot(try(irf(ASvar,impulse=names(ASvar$var$varresult)[im],n.ahead=11)),main=title,ylab=names.plot[1:varchannel$K+1])+title();dev.off();grDevices::png(paste("Fig",j,"8.A model.png",sep="."),type="cairo-png")
plot2<-plot(try(irf(BSvar,impulse=names(BSvar$var$varresult)[im],n.ahead=11)),main=title,ylab=names.plot[1:varchannel$K+1])+title();dev.off()
suppressWarnings(expr = last.warning())
grDevices::png(paste("Fig",j,"9.AB model.png",sep="."),type="cairo-png")
plot3<-plot(try(irf(ABSvar,impulse=names(ABSvar$var$varresult)[im],n.ahead=11)),main=title,ylab=names.plot[1:varchannel$K+1])+title();dev.off()
